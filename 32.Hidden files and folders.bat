...
title Hidden Files & Folders Visibility Manager

:: Read current setting
for /f "tokens=3" %%a in ('reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v Hidden 2^>nul') do (
    set "hidden=%%a"
)
for /f "tokens=3" %%a in ('reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v ShowSuperHidden 2^>nul') do (
    set "superhidden=%%a"
)

:: Determine current status
if "!hidden!"=="0x1" (
    set "status_text=Hidden items are currently: VISIBLE"
    set "status_color=0A"
) else (
    set "status_text=Hidden items are currently: HIDDEN"
    set "status_color=0C"
)
...
if "%choice%"=="2" (
    ...
    color 4F
    echo.
    echo    Setting changed: Hidden files will now be HIDDEN
) else if "%choice%"=="1" (
    ...
    color 2F
    echo.
    echo    Setting changed: Hidden files will now be VISIBLE
) else (
    color 6F
    echo.
...
echo.
echo    =========================================================
echo                 HIDDEN FILES & FOLDERS MANAGER             
echo    =========================================================
color 1F
exit /b
